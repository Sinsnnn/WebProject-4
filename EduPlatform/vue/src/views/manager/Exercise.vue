  ...
  <div>
    <div class="card" style="margin-bottom: 5px">
      <el-input
        ...
        prefix-icon="Search"
        style="width: 240px; margin-right: 10px; margin-bottom: 3px"
        placeholder="请输入题目标题"
      ></el-input>
      <el-select
        v-model="data.search.courseId"
        placeholder="选择课程"
      ...
      <el-select
        v-model="data.search.difficulty"
        placeholder="难度"
        style="width: 120px; margin-right: 10px"
      >
      ...
      <el-button type="warning" plain style="margin-right: 10px" @click="reset"
        >重置</el-button
      >
    </div>
    <div class="card" style="margin-bottom: 5px">
    ...
    <div class="card" style="margin-bottom: 5px">
      <el-table :data="data.tableData" strip>
        ...
        <el-table-column label="难度" prop="difficulty">
          <template v-slot="scope">
            <el-tag v-if="scope.row.difficulty === 0" type="success"
              >简单</el-tag
            >
            <el-tag v-else-if="scope.row.difficulty === 1" type="warning"
              >中等</el-tag
            >
            <el-tag v-else-if="scope.row.difficulty === 2" type="danger"
              >困难</el-tag
            >
          </template>
        </el-table-column>
    ...
    >
      <el-form :model="data.form" label-width="80px" style="padding: 20px 30px">
        ...
        <el-form-item label="参考答案" prop="answer">
          <el-input
            v-if="data.form.type !== 0 && data.form.type !== 2"
...
// 加载课程数据
const loadCourses = () => {
  request.get(courseApi + "/selectMyCourses").then((res) => {
    if (res.code === "200") {
